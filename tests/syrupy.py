...
class OpenRouterSnapshotSerializer(AmberDataSerializer):
    """OpenRouter snapshot serializer for Syrupy.

    Handles special cases for OpenRouter data structures.
    """
...
class OpenRouterSnapshotExtension(AmberSnapshotExtension):
    """OpenRouter extension for Syrupy."""

    VERSION = "1"
    """Current version of serialization format.

    Need to be bumped when we change the OpenRouterSnapshotSerializer.
    """

    serializer_class: type[AmberDataSerializer] = OpenRouterSnapshotSerializer
